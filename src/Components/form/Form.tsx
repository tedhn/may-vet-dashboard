...
const Form: FC<PropTypes> = ({
	...
	const [validInput, setValidInputs] = useState(false);

	useEffect(() => {
		const validation = Object.values(formData).map((data) => {
			console.log(typeof data);
			return data === "";
		});

		if (validation.filter((results) => results !== false).length === 0) {
			setValidInputs(true);
		}
	}, [formData]);
	...
	return (
		<div className='relative w-5/12 px-2 py-8 text-center -translate-x-1/2 -translate-y-1/2 bg-white top-1/2 left-1/2'>
			...
			<div className='flex items-center justify-center gap-4'>
				<button
					className='px-6 py-2 text-white rounded-md w-44'
					style={
						buttonPrimary === "Save"
				...
				<button
					onClick={closeForm}
					className='px-6 py-1 border-2 rounded-md w-44 text-lightGrey'>
					Cancel
				</button>
			</div>
		</div>
